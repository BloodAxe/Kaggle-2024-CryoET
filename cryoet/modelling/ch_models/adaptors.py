...
class MdlCh20dCe2_resnet34(FakeObjectDetectionAdapter):
    def __init__(self):
        super().__init__()
        backbone_args = dict(spatial_dims=3, in_channels=1, out_channels=6, backbone="resnet34", is_pad=False, pretrained=False)
...
class MdlCh20dCe2_effnetb3(FakeObjectDetectionAdapter):
    def __init__(self):
        super().__init__()
        backbone_args = dict(
            spatial_dims=3, in_channels=1, out_channels=6, backbone="efficientnet-b3", is_pad=False, pretrained=False
        )
        self.backbone = FlexibleUNet(**backbone_args)
