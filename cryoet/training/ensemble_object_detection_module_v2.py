...
class EnsembleObjectDetectionModelV2(L.LightningModule):
    def __init__(
        self,
        *,
    ...
    def forward(self, volume, labels=None, **loss_kwargs):
        ...
        for i in range(num_feature_maps):
            averaged_logits = torch.stack([output.logits[i] for output in outputs], dim=0).mean(dim=0)
            logits.append(averaged_logits)

            averaged_offsets = torch.stack([output.offsets[i] for output in outputs], dim=0).mean(dim=0)
            offsets.append(averaged_offsets)

        loss, loss_dict = object_detection_loss(
            logits,
            offsets,
            strides,
            labels=labels,
