...
do
  ...
  torchrun --standalone --nproc-per-node=3 train_od.py \
    --model_name=segresnetv2 --train_spatial_window_size=64 --train_depth_window_size=64 --num_crops_per_study=512 \
    --pretrained_backbone_path=pretrained/wholeBody_ct_segmentation/models/model.pt \
    --per_device_train_batch_size=32 --per_device_eval_batch_size=8 --learning_rate=1e-4 --weight_decay=0.01 \
    ...
    --use_6_classes=True --use_offset_head=False --use_cross_entropy_loss=True --ema=True --ema_beta=4
